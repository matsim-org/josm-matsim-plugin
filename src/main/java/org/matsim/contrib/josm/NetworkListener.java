...
class NetworkListener implements DataSetListener,
    ...
    public NetworkListener(DataSet data, EditableScenario scenario,
	...
	void finished() {
	    ConvertVisitor visitor = new ConvertVisitor();
	    for (Node node : OsmPrimitive.getFilteredList(primitives,
		    Node.class)) {
		visitor.visit(node);
	    }
	    ...
	    for (Relation relation : OsmPrimitive.getFilteredList(primitives,
		    Relation.class)) {
		visitor.visit(relation);
	    }
	}
	...
	for (OsmPrimitive primitive : changed.getPrimitives()) {
		...
		if (links != null) {
		    for (Link link : links) {
			aggregatePrimitivesVisitor.visit((Node) data
				.getPrimitiveById(
					Long.parseLong(link.getFromNode()
						.getId().toString()),
					OsmPrimitiveType.NODE));
			aggregatePrimitivesVisitor.visit((Node) data
				.getPrimitiveById(
					Long.parseLong(link.getToNode().getId()
						.toString()),
					OsmPrimitiveType.NODE));
		    }
		}
	...
	if (links != null) {
		...
		aggregatePrimitivesVisitor.visit((Node) data.getPrimitiveById(
			Long.parseLong(link.getFromNode().getId().toString()),
			OsmPrimitiveType.NODE));
		aggregatePrimitivesVisitor.visit((Node) data.getPrimitiveById(
			Long.parseLong(link.getToNode().getId().toString()),
			OsmPrimitiveType.NODE));
	    }
	}
	...
	// it.
	for (TransitLine line : scenario.getTransitSchedule().getTransitLines()
		.values()) {
    ...
    public void preferenceChanged(
	    org.openstreetmap.josm.data.Preferences.PreferenceChangeEvent e) {
	...
	private EditableTransitRoute createTransitRoute(Relation relation,
		EditableTransitRoute oldRoute) {
		...
		Map<Id<TransitStopFacility>, TransitRouteStop> routeStops = new HashMap<Id<TransitStopFacility>, TransitRouteStop>();
		NetworkRoute networkRoute = null;
		if (Main.pref.getBoolean("matsim_transit_lite")) {
		    for (RelationMember member : relation.getMembers()) {
			if ((member.isNode() && member.getMember().hasTag(
				"public_transport", "stop_position"))
				|| ((member.isNode() || member.isWay()) && member
					.getMember().hasTag("public_transport",
						"platform"))) {
			    TransitStopFacility facility = findOrCreateFacilityLite(member
				    .getMember());
			    if (facility != null) {
				if (!routeStops.containsKey(facility.getId())) {
				    routeStops.put(
					    facility.getId(),
		    ...
		    for (RelationMember member : relation.getMembers()) {
			if (member.isNode()
				&& member.getMember().hasTag(
					"public_transport", "platform")) {
			    TransitStopFacility facility = findFacilityForPlatform(member
				    .getNode());
			    if (facility != null) {
				routeStops
					.put(facility.getId(),
		...
		if (oldRoute == null) {
		    Id<TransitRoute> routeId = Id.create(
			    relation.getUniqueId(), TransitRoute.class);
		...
		newRoute.getStops().addAll(routeStops.values());
		newRoute.setDescription(relation.get("route"));
		String ref = relation.get("ref");
		newRoute.setRealId(ref != null ? Id.create(ref,
			TransitRoute.class) : newRoute.getId());
		return newRoute;
	    } else {
	...
	private TransitStopFacility findOrCreateFacilityLite(
	    ...
	    String id = (primitive.getName() == null ? String.valueOf(primitive
		    .getUniqueId()) : primitive.getName());
	    Id<TransitStopFacility> transitStopFacilityId = Id.create(id,
		    TransitStopFacility.class);
	    if (scenario.getTransitSchedule().getFacilities()
		    .containsKey(transitStopFacilityId)) {
		return scenario.getTransitSchedule().getFacilities()
			.get(transitStopFacilityId);
	    }
	    EastNorth eN = null;
	    if (primitive instanceof Way) {
	    ...
	    TransitStopFacility stop = scenario
		    ...
		    .createTransitStopFacility(transitStopFacilityId,
			    new CoordImpl(eN.getX(), eN.getY()), true);
	    stop.setName(id);
	...
	private TransitLine getTransitLine(Relation relation) {
	    Id<TransitLine> transitLineId = NewConverter
		    .getTransitLineId(relation);
	    EditableTransitLine tLine;
	    if (!scenario.getTransitSchedule().getTransitLines()
		    .containsKey(transitLineId)) {
		tLine = new EditableTransitLine(transitLineId);
		scenario.getTransitSchedule().addTransitLine(tLine);
	    } else {
		tLine = scenario.getTransitSchedule().getEditableTransitLines()
			.get(transitLineId);
	    }
	    Relation maybeLineRelation = ((Relation) data.getPrimitiveById(
		    Long.parseLong(transitLineId.toString()),
	...
	TransitStopFacility findFacilityForPlatform(Node node) {
		...
		if (referrer instanceof Relation
		    ...
		    TransitStopFacility facility = scenario
			    .getTransitSchedule()
			    .getFacilities()
			    .get(Id.create(referrer.getUniqueId(),
				    TransitStopFacility.class));
		    if (facility != null) {
			return facility;
		    }
	...
	private NetworkRoute createNetworkRoute(Relation relation) {
		...
		for (Way way : relation.getMemberPrimitives(Way.class)) {
			...
			if (connections.get(i).direction
				.equals(Direction.FORWARD)) {
			    for (Link link : wayLinks) {
				if (!link.getId().toString().endsWith("_r")) {
				    links.add(link.getId());
				}
			    }
			} else if (connections.get(i).direction
				.equals(Direction.BACKWARD)) {
	...
	public boolean match(Node node) {
	    ...
	    for (Way way : OsmPrimitive.getFilteredList(node.getReferrers(),
		    Way.class)) {
		if ((way.hasKey(NewConverter.TAG_HIGHWAY) || NewConverter
			.meetsMatsimReq(way.getKeys())) && way.isUsable()) {
		    if (way.isFirstLastNode(node) || Preferences.isKeepPaths()
			    || junctionWay != null) {
			return true;
		    } else {
	...
	if (maybeRelation instanceof Relation
		...
		for (EditableTransitRoute transitRoute : editableTransitLine
			.getEditableRoutes().values()) {
		    if (transitRoute.getId().toString()
			    .equals(Long.toString(maybeRelation.getUniqueId()))) {
			return transitRoute;
		    }
		}
	    }
