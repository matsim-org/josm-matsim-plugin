...
class NetworkListener implements DataSetListener,
	...
	class ConvertVisitor extends AbstractVisitor {
		...
		private EditableTransitRoute createTransitRoute(Relation relation,
			...
			if (!relation.isDeleted() && relation.hasTag("type", "route")) {
				...
				if (relation.hasKey("from")
					...
					ref+="_"+ String.valueOf(relation.getUniqueId());
				}
				newRoute.setRealId(ref != null ? Id.create(ref + direction,
		...
		private TransitStopFacility findOrCreateFacilityLite(
			...
			String name = (primitive.getName() == null ? String
					.valueOf(primitive.getUniqueId()) : primitive.getName()+"_"+
					+ primitive.getId());
	...
	private boolean isStop(RelationMember member) {
		return (member.isNode() && member.getMember().hasTag(
				"public_transport", "stop_position"))
				|| ((member.isNode() || member.isWay()) && member.getMember()
