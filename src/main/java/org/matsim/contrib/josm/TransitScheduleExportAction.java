...
class TransitScheduleExportAction extends DiskAccessAction implements org.openstreetmap.josm.data.Preferences.PreferenceChangedListener {
	...
	if (isEnabled()) {
	    File file = createAndOpenSaveFileChooser(tr("Export transit schedule"), new ExtensionFileFilter("xml", "xml", "MATSim Transit Schedule Files (*.xml)"));
	    if (file != null) {
		...
		for (TestError error : test.getErrors()) {
			...
			JOptionPane.showMessageDialog(Main.parent, "Export failed due to validation errors. See validation layer for details.", "Failure", JOptionPane.ERROR_MESSAGE, new ImageProvider("warning-small").setWidth(16).get());
	    ...
	    JOptionPane.showMessageDialog(Main.parent, tr("Nothing to export. Get some data first."), tr("Information"), JOptionPane.INFORMATION_MESSAGE);
	}
    }
	...
	return getEditLayer() instanceof MATSimLayer && ((MATSimLayer) getEditLayer()).getScenario().getConfig().scenario().isUseTransit();
    }
